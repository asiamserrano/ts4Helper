...
//            default: { yield getDefaultURLs(String.format("unknown how to handle %s", url)); }
    ...
    private List<URL> getURLs(String content) {
        switch (this) {
            case PATREON_POSTS: {
...
//                return getDefaultURLs(String.format("unknown how to handle %s content: %s", this.domain, content));
    ...
    private List<URL> getURL(String string) {
        return getURLs(Collections.singletonList(string));
    }

    private List<URL> getURLs(List<String> list) {
        try {
            ...
            return getDefaultURLs(String.format("could not get urls from list: [%s]", strings));
        }
    }
