...
//
//        String string = "https://www.curseforge.com/sims4/create-a-sim/ssalon-female-hairstyle-sh-39";
//        URL url = URLUtility.createURL(string);
//        String content = OkHttpUtility.getContent(url, new OkHttpClient());
//        System.out.println(content);
////        String content = StringUtility.loadResource("examples/CURSE_FORGE_MEMBERS.html");
////        URLModel urlModel = new URLModel(URLUtility.createURL("https://www.curseforge.com/members/ssalon1/projects?page=3&pageSize=20&sortBy=ReleaseDate&sortOrder=Desc"), EMPTY);
//////        URL url = URLUtility.createURL("https://alenaivanisova.my.curseforge.com/?projectsPage=1&projectsSearch=&projectsSort=9");
//////        URLModel urlModel = new URLModel(url, EMPTY);
//////
//////        List<URLModel> list = getCurseForgAPIeNamedURLs(urlModel, content);
//////        for (URLModel urlModel1 : list) System.out.println(urlModel1.url);
////
////        String url_head = WebsiteEnum.CURSE_FORGE_CAS.getHttpUrl().toString();
////        List<URLModel> list =  StringUtility.getSetBetweenRegex(content, "<a class=\" download-cta btn-cta\" href=\"/", "/download")
////                .stream().map(str -> URLUtility.createURL(url_head + str.replace(DomainPath.S4_CAS.value, EMPTY)))
////                .map(url -> new URLModel(url, EMPTY, urlModel))
////                .toList();
////
////        for(URLModel s: list) System.out.println(s.url);
//
//    }
//
//    // https://www.curseforge.com/members/ssalon1/projects?page=3&pageSize=20&sortBy=ReleaseDate&sortOrder=Desc
//    // https://www.curseforge.com/sims4/create-a-sim/ssalon-female-hairstyle-ss137/download/6076113
//    private static List<URLModel> getCurseForgAPIeNamedURLs(URLModel urlModel, String content) {
//        String url_head = WebsiteEnum.CURSE_FORGE_CAS.getHttpUrl().toString();
//        return StringUtility.getSetBetweenRegex(content, "<a class=\" download-cta btn-cta\" href=\"/", "\">")
//                .stream().map(str -> {
//                    URL url = URLUtility.createURL(url_head + str.replace(DomainPath.S4_CAS.value, EMPTY));
//                    String name = str.split("/")[2];
//                    return new URLModel(url, name, urlModel);
//                }).toList();
//    }
//
////    private static List<URLModel> getCurgeForgeEdgeNamedURLs2(URLModel urlModel, String content) {
...
////    }
//
//    private static List<NamedURL> getPatreonNamedURLs(String source_string) {
//        URL source = URLUtility.createURL(source_string);
//        String content = OkHttpUtility.getContent(source, new OkHttpClient());
